...
export async function GET(
  request: Request,
  context: { params: { merchantId: string; promotionName: string } }
) {
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_API_BASE_URL}/schedule-job/${merchantId}/${promotionName}`,
      {
        headers: {
    ...
    if (!response.ok) {
      throw new Error(`Error scheduling job: ${response.status}`);
    }
